...
inline int for_each(TupleT&& tup, UnaryCallbackT&& unary_cb, std::index_sequence<Indices...> _)
{
...
inline int for_each(Tuple1T&& tup1, Tuple2T&& tup2, BinaryCallbackT&& binary_cb, std::index_sequence<Indices...> _)
{
...
template <typename TupleT, typename UnaryCallbackT> inline void for_each(TupleT&& tup, UnaryCallbackT&& unary_cb)
{
  static constexpr std::size_t N = std::tuple_size_v<std::remove_reference_t<TupleT>>;

  detail::for_each(std::forward<TupleT>(tup), std::forward<UnaryCallbackT>(unary_cb), std::make_index_sequence<N>{});
}

template <typename Tuple1T, typename Tuple2T, typename BinaryCallbackT>
inline void for_each(Tuple1T&& tup1, Tuple2T&& tup2, BinaryCallbackT&& binary_cb)
{
