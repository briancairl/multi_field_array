...
template <typename... Ts, typename AllocatorT> class MultiFieldArray<Fields<Ts...>, AllocatorT>
{
public:
  /// Allocators for all \c Ts
  using allocator_tuple_type = tuple_allocator_rebind_t<AllocatorT, Ts...>;
  ...
  explicit MultiFieldArray(const allocator_tuple_type& allocators) : allocators_{allocators}, size_{0}, capacity_{0}
  {
    tuple_for_each(data_, [](auto& ptr) { ptr = nullptr; });
  }
  ...
  {
    MultiFieldArray::allocate_and_default_construct(data_, count);
  }

  MultiFieldArray(std::size_t count, const allocator_tuple_type& allocators) :
  ...
  MultiFieldArray(const MultiFieldArray& other) :
      ...
      capacity_{other.capacity_}
  {
    ...
    else
    {
      ...
      MultiFieldArray::allocate(data_, capacity_);
  ...
  MultiFieldArray(MultiFieldArray&& other) :
   ...
   * \n
   *        @code{.cpp}
   *        MultiFieldArray<Fields<float, int, std::string>> array;
   *
   *        array.emplace_back(
  ...
  inline void emplace_back(std::piecewise_construct_t _, PiecewiseFieldCTorTupleTs&&... ctor_args)
  {
    ...
    MultiFieldArray::check_and_realloc_for_element_added();
  ...
  void resize(const std::size_t new_size)
  {
    ...
    else if (new_size >= capacity_)
    {
      ...
      MultiFieldArray::allocate_and_default_construct(new_data, new_size);

      // Destroy old elements
      MultiFieldArray::destroy(data_);

      // Deallocate old buffers
      MultiFieldArray::deallocate(data_);
   ...
   */
  template <std::size_t Index> inline auto* end() { return MultiFieldArray::template begin<Index>() + size_; }

  /**
   * @copydoc end
   */
  template <typename ValueT> inline ValueT* end() { return MultiFieldArray::template begin<ValueT>() + size_; }
  ...
  {
    return MultiFieldArray::template begin<Index>() + size_;
  }
  ...
  {
    return MultiFieldArray::template begin<ValueT>() + size_;
  }

  /**
   * @brief Returns an iterable data view for one or more types contained within the original array
   */
  template <typename... ViewValueTs> MultiFieldArrayView<Fields<ViewValueTs...>> view()
  {
    return MultiFieldArrayView<Fields<ViewValueTs...>>{std::forward_as_tuple(std::get<ViewValueTs*>(data_)...), size_};
  }
...
private:
  ...
  inline void allocate_and_default_construct(std::tuple<Ts*...>& buffers, const std::size_t capacity)
  {
    // Allocate new memory with specified capacity
    MultiFieldArray::allocate(buffers, capacity);
  ...
  inline void deallocate(std::tuple<Ts*...>& buffers)
  {
    tuple_for_each(allocators_, buffers, [capacity = capacity_](auto& allocator, auto& ptr) {
      allocator.deallocate(ptr, capacity);
      ptr = nullptr;
    });
  };

  inline void move_existing(std::tuple<Ts*...>& buffers)
  {
    tuple_for_each(data_, buffers, [s = size_](auto& src_ptr, auto& dst_ptr) {
      using ElementType = pointer_element_t<decltype(dst_ptr)>;
      ...
      {
        std::move(src_ptr, src_ptr + s, dst_ptr);
      }
    });
  }

  inline void destroy(std::tuple<Ts*...>& buffers)
  {
    tuple_for_each(buffers, [s = size_](auto& ptr) {
      using ElementType = pointer_element_t<decltype(ptr)>;
  ...
  inline void check_and_realloc_for_element_added()
  {
    ...
    MultiFieldArray::allocate(new_data, new_capacity);

    // Move old elements to new buffer
    MultiFieldArray::move_existing(new_data);
  ...
  /// Allocator
  allocator_tuple_type allocators_;
